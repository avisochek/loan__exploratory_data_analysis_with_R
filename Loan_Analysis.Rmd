...
loanData<-
  read.csv("/home/allan/Desktop/loan__exploratory_data_analysis_with_R/prosperLoanData.csv")
```
...
loanData$HasProsperRating <-
   !(loanData$ProsperRating..Alpha.==''|
       is.na(loanData$ProsperRating..Alpha.))
...
ggplot(aes(y=BorrowerRate,x=LoanOriginalAmount),
       data=loanData)+
  geom_point()
```
...
ggplot(aes(y=BorrowerRate,
           x=reorder(BorrowerState, BorrowerRate, FUN=median) ),
       data=loanData)+
  geom_boxplot()+
  coord_flip()+
  xlab("State")
...
loanData$year<- factor(substr(loanData$LoanOriginationQuarter,4,7))
