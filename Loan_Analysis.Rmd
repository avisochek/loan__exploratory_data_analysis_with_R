...
#####most debt levels are in the range of 0 to 1

```{r echo=FALSE, Univariate_Plots1.15,warning=FALSE}
...
ggplot(aes(y=BorrowerRate,x=IncomeRange),data=subset(loanData,!is.na(LoanPeriod)))+
  geom_boxplot()
```

The borrower rate goes down as income range goes up, the exception being the $0 category.

Its likely the $100+ category got mislabeled as Not Employed. We'll just have to ignore it for now.
...
#####Unemployed borrowers tend to have a significantly higher median that the rest of the other borrowers...
...
```{r echo=FALSE, Multivariate_Plots5.23252rew22,message=FALSE}
...
ggplot(aes(x=CreditGrade,y=log10(StatedMonthlyIncome)),
data=subset(loanData,CreditGradeOrProsperRating==TRUE))+geom_boxplot()

ggplot(aes(x=ProsperRating..Alpha.,y=StatedMonthlyIncome),
data=subset(loanData,CreditGradeOrProsperRating==FALSE))+geom_boxplot()+coord_cartesian(ylim=c(500,10000))
```
...
ggplot(aes(y=BorrowerRate,x=AvailableMonthlyIncome),
  ...
  geom_point(alpha=1/20)+
  facet_wrap(~LoanOriginationQuarter)+
  scale_x_log10(limits=c(500,5000))
...
## Employment Status
```{r echo=FALSE, Multivariate_Plots1853278yhuf,message=FALSE,warning=FALSE}
ggplot(aes(x=EmploymentStatus,y=BorrowerRate),data=loanData)+
  geom_boxplot()+
  facet_wrap(~LoanOriginationQuarter)+
   theme(axis.text.x=element_text(angle = 45, hjust = 1))
```
...
```{r echo=FALSE, Multivariate_Plots10,message=FALSE,warning=FALSE}
ggplot(aes(y=BorrowerRate,x=reorder(Occupation, BorrowerRate,FUN=median )),data=loanData)+
...
## Synthesis and Model
```{r echo=FALSE,Multivariate_PlotsjasxzVzsfhdugfheuu,}
# fitq<-with(subset(loanData,
#             CreditGradeOrProsperRating==TRUE&CreditGrade %in% c("AA","A","B")),
#      lm(BorrowerRate~CreditScoreRangeLower+LoanOriginalAmount+10))
# 
# summary(fitq)
# 
# ggplot(aes(y=BorrowerRate,x=(LoanOriginalAmount*.03-CreditScoreRangeLower)),
#        data=subset(loanData,
#             CreditGradeOrProsperRating==TRUE&CreditGrade=="AA"))+
#   geom_point(alpha=1/10)+
#   facet_wrap(~LoanOriginationQuarter)
# 
# ggplot(aes(y=BorrowerRate,
#            x=log10(LoanOriginalAmount)*75-CreditScoreRangeLower+log10(AvailableMonthlyIncome)*50),
#        data=subset(loanData,
#             CreditGradeOrProsperRating==TRUE&CreditGrade=="HR"))+
#   geom_point(alpha=1/10)+
#   facet_wrap(~LoanOriginationQuarter)
# 
# 
...
#   facet_wrap(~LoanOriginationQuarter)
# 
# ggplot(aes(x=log10(LoanOriginalAmount+10)),data=loanData)+
#   geom_histogram(aes(y=..density..),binwidth=.1)+
#   facet_wrap(~LoanOriginationQuarter)
```

```{r echo=FALSE,Multivariate_Plotsjashdugfheuu,}
...
ggplot(aes(x=LoanOriginalAmount,y=BorrowerRate),
  ...
  geom_smooth(method='lm',aes(color=CreditGrade))
